...
**Вывод для Seamster:** генерировать западное ядро + поля-заглушки под лекала/раскладку/расход («предоставляется конструктором»), терминология двуязычная.
...
- Для Seamster: уметь ссылаться на DXF-файл в техпаке + знать словарь слоёв для валидации/превью. Нативные форматы CAD (Gerber `.zip/.plt`, Lectra `.mdl/.iba`, Optitex `.pds`) — passthrough. **[GAP]** — расширения не верифицированы fetch'ем.
