...
**Вывод для Seamsterly:** генерировать западное ядро + поля-заглушки под лекала/раскладку/расход («предоставляется конструктором»), терминология двуязычная.
...
- Для Seamsterly: уметь ссылаться на DXF-файл в техпаке + знать словарь слоёв для валидации/превью. Нативные форматы CAD (Gerber `.zip/.plt`, Lectra `.mdl/.iba`, Optitex `.pds`) — passthrough. **[GAP]** — расширения не верифицированы fetch'ем.
