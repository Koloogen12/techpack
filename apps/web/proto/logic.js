...
class Component extends DCLogic {
   ...
   */
  /**
   * Есть ли у работы технический эскиз.
   *
   * Список файлов работы уже приехал — отдельный запрос за тем же фактом
   * только добавил бы гонку между двумя ответами. Спрашивают об этом три
   * места, и ответ обязан быть один.
   */
  hasSketch() {
  ...
  /**
   * Картинка вида: эскиз по узлам этой вещи, пока он есть; библиотечный
   * силуэт следом; параметрический мастер — только без обоих. Единственная
   * точка, где кабинет решает, что показать человеку, — иначе мастер
   * просачивается через боковые пути.
   *
   * Эскиз отвечает только за «все виды»: он приходит одним листом с обоими
   * видами и на отдельные не делится. Порознь виды остаются силуэтом —
   * у него есть части, а значит слои и выноски.
   */
  viewUrl(view, layers) {
    const s = this.state;
  ...
  quickLook(file) {
    ...
    fetch('/app/api/quicklook', {
      ...
      .then((q) => {
        ...
        // Опорный предмет — единственное, что превращает назначенный масштаб
        // в измеренный. Сказать об этом сейчас, пока человек у экрана с
        // вещью в руках, стоит секунды; сказать после документа — поздно.
        if (q.scale_object && !q.scale_object.present) {
          setTimeout(
            () =>
              this.showToast(
                'Положите рядом лист А4 или карту и переснимите — тогда замеры снимутся с фото, а не подставятся типовыми',
              ),
            3200,
          );
        }
  ...
  renderVals() {
    ...
    const galMap = {
      flat: {
        ...
        b: liveOn
          ? this.hasSketch()
            ? '2 вида · эскиз по узлам этой вещи'
            : s.silh && s.silh.id
    ...
    return {
      ...
      flatHint: !liveOn
        ...
        : this.hasSketch() && s.view === 'all'
          ? 'Эскиз построен по узлам этой вещи и меняется вместе с ними: добавьте или снимите узел — он перерисуется. Размеры берутся из табеля мер, а не с рисунка.'
          : s.silh && s.silh.id
      ...
      silhNote: !s.silh
        ...
        : !s.silh.id
          ...
          : (s.silh.chosen_by_user ? 'Силуэт выбран вами' : 'Силуэт подобран автоматически') +
            // Когда на холсте стоит эскиз, замена силуэта главную картинку
            // не меняет — и полоса обязана сказать, на что она тогда влияет.
