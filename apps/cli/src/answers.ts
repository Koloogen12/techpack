...
export const AnswersSchema = z
  .object({
    ...
    /**
     * Откуда взялась категория в анкете: подставил быстрый взгляд или назвал
     * человек. Полный разбор точнее быстрого; когда человек категорию не
     * трогал, а разбор с неё не согласен, документ собирается по разбору.
     * Названная человеком категория — его слово, и оно сильнее.
     */
    category_source: z.enum(['quicklook', 'user']).optional(),
      ...
      .array(
        z.object({
          ...
          hex_approx: z
            ...
            .optional(),
          /**
           * Имя файла образца полотна в библиотеке бренда.
           *
           * Самый точный вход по цвету: hex говорит о замысле, снимок
           * образца — о том, что бренд держал в руках.
           */
          swatch: text(200).optional(),
          /**
           * Фирменный номер цвета, известный БРЕНДУ. Каталог мы не поставляем:
           * это лицензированные данные. Номер бренда — его собственные данные.
           */
          book_code: text(40).optional(),
        }),
      )
      .max(24)
      .optional(),
