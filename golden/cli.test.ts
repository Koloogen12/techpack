...
describe('кривые входы отвечают человеку, а не стеком вызовов', () => {
  ...
  it('категория вне реестра изделий получает честный отказ', () => {
    try {
      // Пиджак в реестре не заведён: другой класс материала, свои допуски,
      // свой набор узлов. Платье с 3 сентября 2026 поддержано и отказывать
      // на нём больше нечему.
      parseAnswers({ ...ANSWERS, category: 'blazer' });
      expect.unreachable('должно было упасть');
    } catch (e) {
