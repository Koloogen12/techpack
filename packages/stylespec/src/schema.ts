...
export const PatternSpecSchema = z.object({
  ...
  /**
   * Краски раппорта — сколько сеток жечь и какие.
   *
   * Считаются арифметикой по пикселям, а не моделью: печатнику нужен
   * верный список, а не правдоподобный. Код каталога заполняется только
   * если каталог подключён — номера Pantone лицензируемы, и подставлять
   * их из ниоткуда нельзя.
   */
  colors_measured: z.array(
    z.object({
      hex: z.string().regex(/^#[0-9A-F]{6}$/),
      /** Доля площади: от неё зависит расход краски. */
      share: z.number().min(0).max(1),
      book_code: z.string().min(1).nullable(),
      /**
       * Откуда номер краски.
       *
       * `brand` — вписал сам заказчик, это его фирменный цвет и его данные.
       * `catalog` — подобран по подключённому каталогу, и тогда рядом стоит
       * расхождение ΔE. Разница не косметическая: за свой номер отвечает
       * бренд, за подбор отвечаем мы, и печатник вправе знать, что перед ним.
       */
      book_source: z.enum(['brand', 'catalog']).nullable(),
      /** Расхождение с краской каталога, ΔE. null — номер не подбирался. */
      delta_e: z.number().nonnegative().nullable(),
    }),
  ),
  /** Вектор по цветам построен. false — рисунок фотографичен. */
  vector_available: z.boolean(),
  /** Почему вектор есть или почему его нет. Читает человек. */
