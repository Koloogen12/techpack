...
export const MIGRATIONS: readonly Migration[] = [
  ...
  },
  {
    ...
    to: '0.8.0',
    describe:
      'у точки табеля мер появились название и инструкция на английском и китайском; ' +
      'у снапшотов 0.7.0 их нет, и подставить их из справочника нельзя — документ, ' +
      'отправленный фабрике полгода назад, обязан читаться так же, как читался тогда, ' +
      'а формулировка в справочнике с тех пор могла измениться',
    migrate: (snapshot) => ({ ...snapshot, spec_version: '0.8.0' }),
  },
  {
    from: '0.8.0',
    to: '0.9.0',
    describe:
      'у операции техпоследовательности появились формулировки на английском и ' +
      'китайском, а маркировка, нанесение и колорвеи стали частью нерусского ' +
      'комплекта; у снапшотов 0.8.0 переводов операций нет — раздел ' +
      'техпоследовательности в их китайском комплекте останется пустым, и это ' +
      'честнее, чем подставить сегодняшнюю формулировку в прошлогодний документ',
    migrate: (snapshot) => ({ ...snapshot, spec_version: '0.9.0' }),
  },
  {
    from: '0.9.0',
    to: '0.10.0',
    describe:
      'добавлен раздел дизайн-признаков с фото — окат, пояс, клинья, декоративные ' +
      'строчки; у снапшотов 0.9.0 его нет, и восстановить его неоткуда: снимки ' +
  ...
  },
  {
    ...
    to: '0.12.0',
    describe:
      'у узла конструкции появилась отметка «класс стежка задан брендом», у ' +
      'макета нанесения — вид и смещение от середины переда; у снапшотов ' +
      '0.11.0 их нет, и это верно: стежок там справочный, а макет стоит по центру',
    migrate: (snapshot) => ({ ...snapshot, spec_version: '0.12.0' }),
  },
  {
    from: '0.12.0',
    to: '0.13.0',
    describe:
      'у точки табеля появилось место замера на рисунке этой вещи (вид и ' +
      'точки в долях габарита), у дизайна — наблюдения по закрытым словарям; ' +
      'у снапшотов 0.12.0 их нет, и это верно: линии стоят по типовым местам, ' +
      'а узлы тогда выбирались по словам',
    migrate: (snapshot) => ({ ...snapshot, spec_version: '0.13.0' }),
  },
];
