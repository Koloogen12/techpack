...
/**
 * Цепочка по умолчанию: в голове GPT Image 2.5 Flare через Images API,
 * дальше Gemini — от самой способной к тем, что реже блокируют и быстрее
 * прогреваются.
 *
 * Голова выбрана по A/B 09.09.2026 на трёх паках одним промптом: Flare рисует
 * перечисленные в спеке узлы (люверсы 3/3, строчки подгиба и манжеты, рубчик
 * резинки), держит фон по промпту, отвечает за 19 с против 23–34 и стоит
 * вдвое дешевле. Gemini объёмнее, но узлы пропускает — а страница «Внешний
 * вид» существует ради них (ADR-0005). Gemini остаётся запасной: Images API
 * тоже умеет отказывать, и цепочка нужна ровно на этот случай.
 *
