...
export interface Messages {
  ...
  /**
   * Оговорка под машинным эскизом.
   *
   * Эскиз рисуется по узлам ИМЕННО ЭТОЙ спецификации, поэтому показывает
   * и карман, и шнур, и рибану — то, о чём библиотечный силуэт вынужден
   * молчать. Но рисует его модель, а не построение: пропорции на нём
   * приблизительны, и при расхождении с разделом конструкции верен раздел.
   */
  flats_sketch_note: string;
  /** Подпись об источнике эскиза — рядом с идентификатором силуэта. */
  flats_sketch_source: string;
  /**
  ...
  /**
   * Референс рядом с эскизом: подпись колонки снимков заказчика и оговорка.
   *
   * Эскиз рисует модель, и ошибиться она может в узле. Единственный способ
   * увидеть ошибку за секунду — положить рядом то, с чего рисовали. Порознь
   * (эскиз на одном листе, снимки на другом) расхождение не замечает никто.
   */
  flats_reference_label: string;
  flats_reference_note: string;
  /** Подпись снимка, ракурс которого не объявлен. */
