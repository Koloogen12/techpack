import { assume, fromBase, roundCm, userInput, type Tracked } from '@seamster/core';
import {
  kb as defaultKb,
  type Category,
  type KnowledgeBase,
...
import type { Artwork, ArtworkCheck, ArtworkPlacement } from '@seamster/stylespec';
...
function placement(
  ...
  return {
    ...
    ...(zone.label_en ? { zone_label_en: zone.label_en } : {}),
    ...(zone.label_zh ? { zone_label_zh: zone.label_zh } : {}),
    technique,
    ...
    anchor_label_en:
      zone.anchor === 'hps' ? 'down from the HPS' : 'down the sleeve from the shoulder seam',
    anchor_label_zh: zone.anchor === 'hps' ? '自肩颈点向下' : '自肩缝沿袖长向下',
...
export function buildPatternPlacement(
  ...
  return {
    ...
    zone_label_en: 'All-over repeat across the whole garment',
    zone_label_zh: '整件满印循环',
    technique: fromBase(
      technique.id,
    ...
    ...(technique.label_en ? { technique_label_en: technique.label_en } : {}),
    ...(technique.label_zh ? { technique_label_zh: technique.label_zh } : {}),
    ...
    anchor_label_en: 'across the full area, no fixed position',
    anchor_label_zh: '满幅印制，无固定位置',
