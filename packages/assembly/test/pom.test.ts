...
describe('категорийный гейт и трикотажное ядро', () => {
  ...
  it('категория вне ядра получает понятный отказ, а не плохой техпак', () => {
    try {
      // Пиджака в реестре изделий нет: у него другой класс материала,
      // вдвое жёстче допуски и полсотни разделов конструкции вместо
      // тринадцати. Документ вышел бы хуже, чем нужно фабрике.
      buildMeasurements({ ...INPUT, category: 'blazer' as never }, base);
