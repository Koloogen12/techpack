...
describe('состав документа', () => {
  ...
  it('раздел лекал перечисляет детали кроя и говорит, что лекала строит конструктор', () => {
    const doc = html();
    // Перед и спинка есть у любого изделия; лекала мы по-прежнему не строим,
    // и лист об этом говорит — но теперь после перечня, а не вместо него.
    expect(doc).toContain('Перед');
    expect(doc).toContain('Деталей на изделие');
    expect(doc).toContain('Лекала и раскладку строит конструктор');
  });
