...
describe('шаблон точек измерения футболки', () => {
  ...
  it('каждая точка объявляет, за чем следует её величина', () => {
    ...
    // Длина изделия и длина РУКИ следуют за ростом: человек на четыре размера
    // больше не имеет рук на четверть длиннее — он шире. Рукав от плечевой
    // точки за ростом уже не следует: он равен руке минус половина плеч,
    // и спущенное плечо законно его укорачивает.
    for (const code of ['T01', 'T11']) {
      expect(byCode.get(code)!.anchor_basis, code).toBe('height');
    }
    expect(byCode.get('T10')!.derivation).toBe('composed');
