...
export const CareProfileSchema = z
  .object({
    ...
    label_ru: z.string().min(1),
    /**
     * Ровно один вариант на каждую из шести групп ГОСТ ISO 3758.
     *
     * Пропустить группу нельзя, хотя раньше здесь было написано обратное:
     * `z.record` по замкнутому перечислению требует все ключи, и схема
     * всегда работала строже своего комментария. Это не придирка — ярлык
     * с пропущенным символом читается как «производитель забыл», а не
     * как «требования нет». Там, где символ бессмыслен (сушка у вещи,
     * которую нельзя стирать), ставится ближайший корректный, а сам факт
     * объясняется в `gap` профиля.
     */
