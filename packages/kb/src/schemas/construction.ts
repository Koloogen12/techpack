...
export const CutPartSchema = z
  .object({
    ...
    /**
     * Из какого материала режется деталь.
     *
     * Роли совпадают с ролями материала в спецификации (MATERIAL_ROLES):
     * лист деталей берёт из строки состава название полотна, и разойдись
     * эти два перечня — конструктор увидел бы в графе «материал» ключ
     * роли вместо «подкладочная тафта 190Т».
     *
     * `lining` и `insulation` заведены под верхнюю одежду: у куртки
     * полотен три, и деталь подкладки нельзя назвать основной — у неё
     * свой расход, свой крой и своя строка состава по слоям (ТР ТС 017
     * ст. 9).
     */
    role: z.enum(['shell', 'lining', 'insulation', 'rib', 'interlining']),
