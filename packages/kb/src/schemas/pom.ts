...
export const PomPointSchema = z
  .object({
    ...
    /**
     * Узел, без которого точки в изделии нет.
     *
     * Табель мер собирается по категории, а категория описывает семейство:
     * куртка бывает и с капюшоном, и со стойкой. Без этой связи документ
     * печатал куртке со стойкой «высота капюшона 37,4 см» — число, которое
     * ОТК обязан померить у детали, которой нет. Пометить точку
     * необязательной мало: она всё равно печатается со значением.
     *
     * Пусто — точка есть у изделия всегда.
     */
