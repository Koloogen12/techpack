...
export interface FlatMeasurements {
  ...
  /**
   * Ход молнии: по центру переда или асимметрично, от горловины по
   * диагонали к боковому шву. Берётся из дизайн-признака с фото: реестр
   * узлов знает только «втачать молнию», а куда она идёт, видно на снимке.
   * Пусто — по центру.
   */
  zipPath?: 'center' | 'asymmetric';
