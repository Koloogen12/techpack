...
const KIN: Record<Category, readonly Category[]> = {
  ...
  polo: ['tshirt'],
  tank_top: ['tshirt'],
  // Кардиган и свитер — трикотажный верх той же сборки, что свитшот, и
  // покупных силуэтов вязаных изделий в библиотеке нет. Свитшот даёт
  // верную пластику плеча и рукава, а рибану и планку дорисует наш слой.
  // Худи в родню не идёт: капюшон нарисован на самом силуэте, и стереть
  // его с покупного шаблона нечем.
  // Верхняя одежда: в библиотеке 123 листа этого семейства, и они
  // единственные рисуют объём на подкладке — плечо шире, пройма ниже,
  // низ не облегает. Трикотажный верх сюда в родню не идёт: свитшот дал
  // бы силуэт вещи, надетой на тело, а не поверх другой одежды.
  jacket: ['bomber', 'coat'],
  bomber: ['jacket', 'zip_hoodie'],
  // Пальто роднится только с курткой: длина у него своя, а всё, что
  // короче, ставит линию низа не туда.
  coat: ['jacket'],
  cardigan: ['sweatshirt', 'longsleeve'],
  sweater: ['sweatshirt', 'longsleeve'],
  // Родственника у платья в библиотеке нет: покупные силуэты — весь
  // ассортимент стритвира и ни одного платья. Лонгслив дал бы верное плечо
  // и неверную длину, а длина у платья и есть главное. Пусто честнее:
  // лист чертежа собирается на эскизе, который платье рисует как платье.
  // Рубашка и блузка — ближайшая родня друг другу; поло делит с рубашкой
  ...
  // В библиотеке силуэтов брюк 176 записей, но категория у них не проставлена:
  // подбирать по ней нечего, пока разметка не сделана.
  trousers: [],
  skirt: [],
  dress: [],
};
